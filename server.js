...
io.on('connection', (socket) => {
  ...
  socket.on(
    'setup_room',
    ({ roomID, userID, fullName, isLogin, type, avatarURL, time }) => {
      socket.join(roomID);
      const authorInfo = {
  ...
  socket.on('kick_user', ({ roomID, kickedUserID }) => {
    if (rooms.has(roomID)) {
      let users = roomUsers.get(roomID);
      const targetedIndex = users.findIndex(
        (user) => user.authorID === kickedUserID
      );
  ...
  socket.on('send_friend_request', ({ roomID, friendRequest }) => {
    if (rooms.has(roomID)) {
      ...
      const requestedFromUser = users.find(
        (user) => user.authorID === friendRequest.requestedFrom
      );
      delete requestedFromUser.registered;
      delete requestedFromUser.type;

      const beingRequestedUser = users.find(
        (user) => user.authorID === friendRequest.beingRequested
      );
      ...
      io.to(beingRequestedUser.userSocketID).emit(
  ...
  socket.on('send_friend_request_response', (response) => {
    ...
    if (rooms.has(roomID)) {
      io.to(friendRequest.requestedFrom.userSocketID).emit(
        'receive_friend_request_response',
        { isAccepted, friendRequest, youAre: 'requesting' }
      );
      io.to(friendRequest.beingRequested.userSocketID).emit(
        'receive_friend_request_response',
        { isAccepted, friendRequest, youAre: 'requested' }
      );
    }
  });
});
...
app.get('/', async (req, res) => {
  if (req.session.user) {
    const response = await db
      .collection('users')
      .findOne({ _id: new ObjectId(req.session.user._id.toString()) });
...
app.post('/addfriend', async (req, res) => {
  ...
  await db
    ...
    .updateOne(
      ...
      { $push: { friends: { friendID: friendInfo } } }
    );
...
app.patch('/unfriend', async (req, res) => {
  ...
  const targetUser = await db
    .collection('users')
    .findOne({ _id: new ObjectId(target.authorID) });
  const updatedTargetFriends = targetUser.friends.filter(
    (friend) => friend.authorID !== user._id
  );
  await db
    .collection('users')
    .updateOne(
  ...
  const updatedUserFriends = user.friends.filter(
    (friend) => friend.authorID !== target.authorID
  );
  await db
    .collection('users')
    .updateOne(
...
app.get('/conversations', async (req, res) => {
  if (req.session.user) {
    ...
    const result = await db
      ...
      .find({ 'participantsInfo.userID': userID })
      .toArray();
    res.status(200).json(result);
  }
});
...
app.patch('/update_conversation', (req, res) => {
  db.collection('conversations')
    .updateOne(
      ...
      { $push: { messages: req.body.messageData } }
    )
    .then((result) => {
      res.status(200).json(result);
    });
